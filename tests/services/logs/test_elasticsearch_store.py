...
from nv.svc.farm.services.logs.facilities.elasticsearch import ElasticsearchLogStore
...
class TestElasticsearchLogStore(unittest.IsolatedAsyncioTestCase):
    ...
    def _get_log_store(self) -> ElasticsearchLogStore:
    ...
    async def _get_logs(self, store: ElasticsearchLogStore, task_id: str, agent_id: str = None, log_entries=50):
        await asyncio.sleep(1)
        return await store.get_logs(task_id=task_id, agent_id=agent_id, log_entries=log_entries)

    @unittest.skip("Integration test, requires running Elasticsearch server.")
    ...
    async def test_remove_duplicate_log_lines(self):
        store = ElasticsearchLogStore(["foo"])
