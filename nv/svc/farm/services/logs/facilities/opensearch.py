...
"""Farm Logs Service OpenSearch Log Store Facility."""
...
class OpensearchLogStore(BaseLogStore):
    """Opensearch log store."""

    def __init__(self, hosts: List[str] = None, index: str = None, **kwargs) -> None:
        """Opensearch log store constructor."""
        super().__init__()
        if not hosts:
            raise Exception("No hosts given for Opensearch Log store")
    ...
    async def get_logs(self, task_id: str, agent_id: str = None, latest_only: bool = True, log_entries: int = 50) -> Dict:
        ...
        """
        if log_entries == 0:
            # NOTE: 0 means no limit per logs api, but OpenSearch has a limit.
