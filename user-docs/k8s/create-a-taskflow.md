...
```bash
curl -X POST \
  "http://farm.127-0-0-1.nip.io:8080/queue/management/tasks/submit-graph" \
...
Monitor the DAG in the [dashboard](http://farm.127-0-0-1.nip.io:8080/queue/management/dashboard). You'll see:
...
curl "http://farm.127-0-0-1.nip.io:8080/queue/management/results/task/<piglatinize-task-id>"
```
...
Watch the [dashboard](http://farm.127-0-0-1.nip.io:8080/queue/management/dashboard) to see the parallel execution. After `store-node` completes, both transformation tasks start simultaneously (assuming sufficient cluster resources).
...
After all tasks complete, check the final report by viewing the task logs in the [dashboard](http://farm.127-0-0-1.nip.io:8080/queue/management/dashboard) to see the generated report combining both transformation outputs.
...
Now rebuild the `wordctl` container with your new command:

```bash
devspace run-pipeline wordctl
```

This injects the updated `wordctl:dev` image into your cluster. The Job Definition already references this tag, so new tasks will automatically use the updated container.
...
Watch the execution in the [dashboard](http://farm.127-0-0-1.nip.io:8080/queue/management/dashboard). You'll see:
